...
**Solution:** AI-driven citizen reporting platform with real-time severity scoring, interactive hazard maps, municipality dashboards, and sustainability impact tracking.
...
│   ├── services/
│   │   ├── aiService.js         # Detection validation + Groq LLaMA3 integration
│   │   └── roadDamageDetector.js # Node → Python → YOLO bridge
...
│
├── ml/
│   ├── inference.py             # YOLOv8 road-damage inference
│   ├── download_model.py        # Pinned model download + SHA-256 verification
│   ├── requirements.txt
│   └── README.md                # Model card, setup and limitations
│
├── frontend/
│   ├── public/
│   │   └── index.html
...
│   │   │       └── RoadDamageOverlay.js # Responsive YOLO bounding-box visualization
│   │   ├── context/
...
[Detection Validation]
  ...
  - Invalid detections discarded
       │
       ├──────────────► [Responsive React Bounding-Box Overlay]
       │                 - Actual detection boxes
       │                 - Pothole / Crack labels
       │                 - Confidence percentages
       │
       ▼
[Severity Placeholder]
  - TEMPORARY: medium if detected, low if not
  - Not model output; real severity modeling is planned for Phase 3
       │
       ▼
[Groq-hosted Llama 3 8B]
  - System prompt only; no RAG
...
The report detail view now visualizes the actual YOLO detections directly on the uploaded road image. Bounding boxes use normalized 0–1 coordinates, so they remain aligned as the image resizes responsively.

For each valid detection, the UI shows:

- **Damage type:** Pothole or Crack
- **Confidence:** model confidence percentage
- **Bounding box:** the actual YOLO detection location
- **Detection summary:** total issues, pothole/crack counts, and highest confidence

Low-confidence detections below the configured `ROAD_DAMAGE_CONF` threshold (default `0.25`) are not returned by the Python inference pipeline. Invalid detection records are also rejected before they reach the frontend.

Severity remains a temporary placeholder and is **not** inferred from confidence. Real severity modeling is planned for Phase 3.
